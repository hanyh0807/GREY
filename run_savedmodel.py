...
import python_networks.CellFate_Boolean_foldchange.Si_restore as test_prob
...
max_task_num = 100000
scale = 5
env = test_prob.simul(num_initial=5000, start_process=False, max_task_num=max_task_num, use_randsign=use_randsign, scale=scale)
...
for _ in range(3):
    st = time()
    print(_)
    self.val_env.do_initialize_task.value = 0
    self.val_env.seed.value = seed
    self.val_env.mb_size.value = self.val_env.train_y.shape[1]
    np.random.seed()
    asize = self.val_env.get_num_params()[0]
    s = np.zeros(asize) + 0.5

    self.val_env.do_reset_initials.value = 1
    ...
    if self.action_type == 'discrete' or self.action_type == 'select':
        ...
        s_scaled = np.round((s * self.action_width) - self.action_center)

    signmat = np.sign(self.val_env.weightmat[self.val_env.weightmat!=0])
    s = np.random.choice(np.arange(1,self.action_center[0]+1), size=signmat.size)
    s_scaled = s * signmat
    s = s_scaled + self.action_center[0]
    print('initial s',s_scaled)

    self.val_env.params[:asize] = s_scaled.reshape([-1])
    self.val_env.seed.value = seed
    self.val_env.tvt.value = 0
    self.val_env.START_CALC_EVENT.set()
    r = self.val_env.rq.get()
    first_r = r
    spear = self.val_env.sq.get()
    mui = self.val_env.zc.get()
    first_s = spear
    first_mi = mui
    r = r - first_r
    r2 = np.array([spear-first_s, mui-first_mi, 0]).reshape([1,1,-1])
    self.val_env.do_reset_initials.value = 0
    self.val_env.do_initialize_task.value = 0

    cur_dummy_params_num = asize
    cur_dummy_link_idx = np.array(self.val_env.weightmat_link_idx[:].copy())[:int(asize*2)]
    ...
    recon_adj = np.sign(self.val_env.weightmat)
    ...
    for j in range(self.max_ep):
        ...
        for rep in range(repnum):
            ...
            a_dist_c, state_c, context_state_c, gs_state_new_c, gs_est_new_c, msg_nf_new_c, msg_lf_new_c, msg_gf_new_c = sess.run([local_AC_running.policy, local_AC_running.rnn_state, local_AC_running.context_rnn_state, local_AC_running.gs_rnn_state, local_AC_running.gs_estimate, local_AC_running.msg_nf, local_AC_running.msg_lf, local_AC_running.msg_gf],
                                      feed_dict=feed_dict)
            ...
            if j > 10:
                if actrepeat & (np.random.rand()<100):
                    ...
                    gs_state_new_c, gs_est_new_c = sess.run([local_AC_running.gs_rnn_state, local_AC_running.gs_estimate], feed_dict=feed_dict)
            ...
            if self.action_type == 'discrete':
                a_dist_scaled = a_dist_c.reshape([-1]) - self.action_center[0]

            self.val_env.params[:cur_dummy_params_num] = a_dist_scaled.reshape([-1])
            self.val_env.seed.value = seed
            self.val_env.tvt.value = 0
            self.val_env.START_CALC_EVENT.set()
            r_ = self.val_env.rq.get()
            spear = self.val_env.sq.get()
            mui = self.val_env.zc.get()

            print(j, 'cand', rep, r_, 'elap', time()-rst)
            sys.stdout.flush()
            rtemp.append(r_)
            stemp.append(spear)
            mtemp.append(mui)
        ...
        print('step:',j,' reward',r_)
        print('step:',j,' action',a_dist.ravel())
    ...
    qa.append(running_action)
    q.append(np.max(his))
    qoi.append(OI(first_r,np.array(his)))
    qhis.append(his)
    print(_, time() - st, 'Elapsed')

    x = np.arange(self.max_ep)
    fig = tpl.figure()
    fig.plot(x, his)
    fig.show()

with open('afatinib_300ccl_optimize.p','wb') as f:
    pickle.dump({'qa':qa,'q':q,'qoi':qoi,'qhis':qhis},f)
