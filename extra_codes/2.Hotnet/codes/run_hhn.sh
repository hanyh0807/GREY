...
data=$PWD/$data_dir
intermediate=$PWD/${network_name}_intermediate
results=$PWD/${network_name}_results
...
for network in $network_name
do
    python $hhn_home/src/construct_similarity_matrix.py \
...
do
    ...
    parallel -u -j $num_cores --bar \
        python $hhn_home/src/permute_network.py \
...
do
    ...
    do
        ...
        python $hhn_home/src/find_permutation_bins.py \
        ...
        parallel -u -j $num_cores --bar \
            python $hhn_home/src/permute_scores.py \
...
do
    ...
    do
        parallel -u -j $num_cores --bar \
            python $hhn_home/src/construct_hierarchy.py \
...
do
    for score in scores
    do
        python $hhn_home/src/process_hierarchies.py \
...
# python $hhn_home/src/perform_consensus.py \
#     -cf  $results/clusters_{$network_name}_scores_1.tsv $results/clusters_{$network_name}_scores_2.tsv $results/clusters_{$network_name}_scores_3.tsv \
#     -igf $data/{$network_name}_index_gene.tsv $data/{$network_name}_index_gene.tsv \
#     -elf $data/{$network_name}_edge_list.tsv $data/{$network_name}_edge_list.tsv \
#     -n   {$network_name} {$network_name} \
#     -s   scores_1 scores_2 scores_3 \
#     -t   2 \
