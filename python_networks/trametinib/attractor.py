...
def find_attractor_pert_v4(weightmat, initial_state, pert_idx, stim_idx, outmut_initial=False, initial_pert_idx=None, initial_stim_idx=None): #np.array, np.array
    num_initial = len(initial_state)
    tot_att = [[]] * num_initial
    initial_state = np.array(initial_state)
    ...
    while 1:
        ...
        if current_state.shape[0] < 10000:
            weighted_sum = matmul_(current_state, weightmat)
        else:
            weighted_sum = matmul(current_state, weightmat)

        if not outmut_initial and (initial_pert_idx is not None and initial_stim_idx is not None):
            if initial_pert_idx.size > 0:
                weighted_sum[:,initial_pert_idx] += -1
            if initial_stim_idx.size > 0:
                weighted_sum[:,initial_stim_idx] += 1
